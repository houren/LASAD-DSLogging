...
/**
 * VML based implementation of BezierCurve. Do not instantiate directly @see
 * BezierCurve
 * 
 * @author Michał Baliński (michal.balinski@gmail.com)
 * 
 * Modified by LASAD group
 */
public class StraightLineVml extends BezierCurve {
	...
	public void draw(Point p1, Point p2, Point c1, Point c2) {
		...
		// DOM.setElementAttribute(vmlCurve, "strokecolor",
		// getComputedStyle(styledDiv, "color"));
		DOM.setElementAttribute(vmlCurve, "strokecolor", DOM.getStyleAttribute(styledDiv, "color"));

		DOM.setElementAttribute(vmlCurve, "strokeweight", DOM.getStyleAttribute(styledDiv, "width"));
		// DOM.setElementAttribute(vmlCurve, "strokecolor",
		// getComputedStyle(vmlCurve, "color"));
		// DOM.setElementAttribute(vmlCurve, "strokeweight",
		// getComputedStyle(vmlCurve, "width"));

		Point start = new Point(Math.min(p1.left, p2.left), Math.min(p1.top, p2.top));

		DOM.setStyleAttribute(vmlGroup, "left", Integer.toString(start.left - 20));
		DOM.setStyleAttribute(vmlGroup, "top", Integer.toString(start.top - 20));

		drawImpl(p1.move(start.negative()).move(new Point(20, 20)), p2.move(start.negative()).move(new Point(20, 20)), c1.move(start.negative()).move(new Point(20, 20)), c2.move(start.negative()).move(new Point(20, 20)));
	...
	private void drawImpl(Point p1, Point p2, Point c1, Point c2) {
		...
		// Use these lines instead for straight lines.
		DOM.setElementAttribute(vmlCurve, "control1", p1.left + "," + p1.top);
		DOM.setElementAttribute(vmlCurve, "control2", p2.left + "," + p2.top);
	}
